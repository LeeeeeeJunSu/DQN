...
class DQNAgent:
    def __init__(self, replay_buffer_size, warmup_count, batch_size, eps_start, eps_end, eps_decay, network_update_freq, gamma, agent_name, reward_scale, norm_alpha):
        # Hyperparameters
    ...
    def get_action(self, ball_x, ball_y, ball_z, ball_speed_x, ball_speed_y, ball_speed_z, agent_pos_x_list, agent_pos_y_list, gripper_z_list, episode_done):
        self.input_queue.append((ball_x, ball_y, ball_z, ball_speed_x, ball_speed_y, ball_speed_z, agent_pos_x_list, agent_pos_y_list, gripper_z_list, episode_done))
    ...
    def InternalThread(self):
        ...
        while not self.stop_flag:
            # Get State And Convert To Numpy Array
            ball_x, ball_y, ball_z, spd_x, spd_y, spd_z, agent_pos_x_list, agent_pos_y_list, gripper_z_list, done = self.get_state()
            state_vals = [ball_x, ball_y, ball_z, spd_x, spd_y, spd_z]
            state_vals.extend(agent_pos_x_list)
            state_vals.extend(agent_pos_y_list)
            state_vals.extend(gripper_z_list)
            ...
            mean_str = "" if state_mean is None else ",".join(f"{m:.6f}" for m in state_mean)
            std_str = "" if state_std is None else ",".join(f"{s:.6f}" for s in state_std)
            self.step_log.write(f"{update_count},{episode_index},{reward:.4f},{epsilon:.4f},{mean_str},{std_str},{qmax_value:.4f}\n")
            self.step_log.flush()
